...
//    getTemplatesFromFile(fn_templates, h_b, h_c, h_l);
  ...
  // Input percentages from file
    ...
    while(percentFile >> ipB >> ipC >> ipL)
    {
      ...
      // Create output lable
        TString l_output = TString::Format("fit_TEMPS_%s_ON_%s_WITH_RATIOS_%s", l_templates.Data(), l_sample.Data(), l_ratio.Data());
      ...
      // TEMPLATE TEST
        ...
        mainLog << templateFitter(l_output , h_sample, h_b, h_c, h_l, 1);
        cout << mainLog.str();
        break;
    }
