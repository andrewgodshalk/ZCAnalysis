...
{  // Extract DataWeights (raw_eff_plots/<CHANNEL>/<CHANNEL>/n_tagged_<HFTag>)
    ...
    if(channel == "Zuu")
    {
        h_dtwt = (TH1F*) f->Get(path+muDataSets[0]+"/Zuu/ntJet_"+hftag)->Clone("h_dtwt");
        for( int i=1; i<muDataSets.size(); i++ )
        {
            TH1F* h_temp = (TH1F*) f->Get(path+muDataSets[i]+"/Zuu/ntJet_"+hftag)->Clone("h_temp");
            h_dtwt->Add(h_temp);
            delete h_temp;
        }
    }
    ...
    {
        h_dtwt = (TH1F*) f->Get(path+elDataSets[0]+"/Zee/ntJet_"+hftag)->Clone("h_dtwt");
        for( int i=1; i<elDataSets.size(); i++ )
        {
            TH1F* h_temp = (TH1F*) f->Get(path+elDataSets[i]+"/Zee/ntJet_"+hftag)->Clone("h_temp");
            h_dtwt->Add(h_temp);
            delete h_temp;
        }
    }
...
cout << "      getDenominatorPlot(" << f->GetTitle() << ", " << channel << ", " << flavor << ")" << endl;
    TH1F *h_den, *h_temp;
    ...
    if(channel == "Zuu" || channel == "Zee")
    {
        ...
        {
            h_temp = (TH1F*) f->Get(path1j+i+"of"+n+'/'+channel+"_"+flavor+"/n_"+flavor[1]+"Jets")->Clone("h_temp");
            h_den->Add(h_temp);
            delete h_temp;
        }
    }
    ...
    {   h_den  = (TH1F*) f->Get(path+"Zuu_"+flavor+"/n_"+flavor[1]+"Jets")->Clone(hName);
        ...
        for( char i='1', n='7'; i<=n; i++)
        {
            h_temp = (TH1F*) f->Get(path1j+i+"of"+n+'/'+"Zuu_"+flavor+"/n_"+flavor[1]+"Jets")->Clone("h_temp");
            h_den->Add(h_temp);
            delete h_temp;
            h_temp = (TH1F*) f->Get(path1j+i+"of"+n+'/'+"Zee_"+flavor+"/n_"+flavor[1]+"Jets")->Clone("h_temp");
            h_den->Add(h_temp);
            delete h_temp;
        }
    }
...
{ // Extract Num (raw_eff_plots/dy/Zee_Zf/n_tagged_ZfEvents_CSV#)
    ...
    if(channel == "Zuu" || channel == "Zee")
    {
        ...
        for( char i='1', n='7'; i<=n; i++)
        {
            h_temp = (TH1F*) f->Get(path1j+i+"of"+n+'/'+channel+"_"+flavor+"/nt_"+flavor[1]+"Jets_"+hftag+errSuffix)->Clone("h_temp");
            h_num->Add(h_temp);
            delete h_temp;
        }
    }
    ...
    {   h_num  = (TH1F*) f->Get(path  +"Zuu_"+flavor+"/nt_"+flavor[1]+"Jets_"+hftag+errSuffix)->Clone(hName);
        ...
        for( char i='1', n='7'; i<=n; i++)
        {
            h_temp = (TH1F*) f->Get(path1j+i+"of"+n+"/Zuu_"+flavor+"/nt_"+flavor[1]+"Jets_"+hftag+errSuffix)->Clone("h_temp");
            h_num->Add(h_temp);
            delete h_temp;
            h_temp = (TH1F*) f->Get(path1j+i+"of"+n+"/Zee_"+flavor+"/nt_"+flavor[1]+"Jets_"+hftag+errSuffix)->Clone("h_temp");
            h_num->Add(h_temp);
            delete h_temp;
        }
    }
